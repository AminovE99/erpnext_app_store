							...
							for (var field in _data) {
								if (field.includes(':')) {
									...
									var cdt_row = Object.keys(updated_doc)
										.filter(function (key) { return Array.isArray(updated_doc[key]) && updated_doc[key][0].doctype === cdt; })
										.map(function (key) { return updated_doc[key]; })
										.map(function (a) { return a[0]; })
		...
		export_report() {
			...
			this.export_dialog = frappe.prompt([
				...
				if (file_format === 'CSV') {
					...
					var data = this$1.get_data_for_csv();
