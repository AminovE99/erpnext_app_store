(function () {
	...
	frappe.views.ListView = class ListView extends frappe.views.BaseList {
		...
		setup_new_doc_event() {
			...
			this.$no_result.find('.btn-new-doc').click(function () { return this$1.make_new_doc(); });
		}
	...
	frappe.views.ListSidebar = class ListSidebar {
		...
		make() {
			...
			$('.assigned-to').on('click', function () {
				$('.assigned').remove();
				this$1.setup_assigned_to();
			});
		...
		setup_assigned_to() {
			...
			var dropdown = this.page.sidebar.find('.assigned-dropdown');
			var current_filters = this.list_view.get_filters_for_args();

			frappe.call('frappe.desk.listview.get_user_assignments_and_count', {doctype: this.doctype, current_filters: current_filters}).then(function (data) {
				$('.assigned-loading').hide();
				var current_user  = data.message.find(function (user) { return user.name === frappe.session.user; });
				if(current_user) {
					var current_user_count = current_user.count;
					this$1.get_html_for_assigned(frappe.session.user, current_user_count).appendTo(dropdown);
				}
				var user_list = data.message.filter(function (user) { return !['Guest', frappe.session.user, 'Administrator'].includes(user.name) && user.count!==0; } );
				user_list.forEach(function (user) {
					this$1.get_html_for_assigned(user.name, user.count).appendTo(dropdown);
				});
				$(".assigned-dropdown li a").on("click", function (e) {
					var assigned_user = $(e.currentTarget).find($('.assigned-user')).text();
					if(assigned_user === 'Me') { assigned_user = frappe.session.user; }
					this$1.list_view.filter_area.remove('_assign');
					this$1.list_view.filter_area.add(this$1.list_view.doctype, "_assign", "like", ("%" + assigned_user + "%"));
				});
			});
		}
		...
		get_html_for_assigned(name, count) {
			...
			var html = $('<li class="assigned"><a class="badge-hover" href="#" onclick="return false;" role="assigned-item"><span class="assigned-user">'
		...
		setup_upgrade_box() {
			...
			var upgrade_date = frappe.datetime.add_days(frappe.datetime.get_today(), 30);
	...
	frappe.views.FileView = class FileView extends frappe.views.ListView {
		...
		make_new_doc() {
			...
			new frappe.ui.FileUploader({
				...
				on_success: function () { return this$1.refresh(); }
			});
		}
